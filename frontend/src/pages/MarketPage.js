  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4" data-testid="market-page">
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="hidden md:flex items-center space-x-4 mb-8 overflow-x-auto pb-4">
          <button
        ...
        {/* Mobile Filter & Sort Selects */}
        <div className="flex flex-col md:hidden space-y-4 mb-8">
          <select
            className="bg-[#1E1E1E] border border-zinc-800 text-white px-4 py-3 rounded-lg focus:outline-none focus:border-[#FDD500] w-full font-bold uppercase tracking-wide"
            value={selectedCategory || 'Tümü'}
            onChange={(e) => setSelectedCategory(e.target.value)}
          >
            <option value="Tümü">Tümü</option>
            <option value="En Çok Satanlar">En Çok Satanlar</option>
            {categories.map((cat) => (
              <option key={cat.id} value={cat.isim}>{cat.isim}</option>
            ))}
          </select>
        </div>
        ...
        <div className="mb-6 flex justify-end">
          <select
            className="bg-[#1E1E1E] border border-zinc-800 text-white px-4 py-2 rounded-lg focus:outline-none focus:border-[#FDD500] w-full md:w-auto"
