...
const Navigation = () => {
  ...
  const navLinks = [
    ...
    { to: '/cuzdan', label: 'Kredi Yükle' }
  ];

  return (
    <>
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#222222]/90 backdrop-blur-md border-b border-white/5">
          ...
          <div className="hidden lg:flex items-center space-x-14">
            {navLinks.map(link => (
              <Link
                key={link.to}
                to={link.to}
                className={`text-zinc-400 hover:text-[#FDD500] font-medium transition-colors uppercase tracking-wider text-sm whitespace-nowrap ${link.label === 'Kredi Yükle' ? 'bg-[#FDD500]/10 px-4 py-2 rounded-lg border border-[#FDD500]/20 hover:bg-[#FDD500] hover:text-black shadow-md' : 'px-3'}`}
            ...
            {user ? (
              <div className="relative" ref={profileMenuRef}>
                <button
                  ...
                  className="flex items-center space-x-3 px-4 py-2 bg-[#1E1E1E] border border-zinc-800 rounded-xl hover:border-[#FDD500]/50 transition-all shadow-md"
                ...
                <div
                  className={`absolute right-0 top-full mt-2 w-56 bg-[#1E1E1E] border border-zinc-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 origin-top transform ${showProfileMenu ? 'opacity-100 scale-y-100 visible' : 'opacity-0 scale-y-95 invisible'}`}
